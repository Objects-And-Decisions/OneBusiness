...
function ajouterAuPanier(nom, prix, fournisseur, image="", prixOriginal=null, remise=null){
  panier.push({ nom, prix:Number(prix), fournisseur, image,
    prixOriginal: prixOriginal ? Number(prixOriginal) : null,
    remise: remise ? Number(remise) : null });
  savePanier();
...
function afficherPanier(){
  ...
  panier.forEach((item, i)=>{
    ...
    const nomCell = item.remise
      ? `<strong style="color:#0f172a">${item.nom}</strong>
         <span class="panier-solde-badge">🔥 −${item.remise} %</span>`
      : `<strong style="color:#0f172a">${item.nom}</strong>`;

    const prixCell = item.prixOriginal
      ? `<span class="panier-prix-ancien">${formatPrix(item.prixOriginal)}</span>
         <strong class="panier-prix-solde">${formatPrix(item.prix)}</strong>`
      : `<strong>${formatPrix(item.prix)}</strong>`;

    tr.innerHTML = `
      <td>${nomCell}</td>
      <td><span class="badge badge-green">${item.fournisseur}</span></td>
      <td><div class="panier-prix-wrap">${prixCell}</div></td>
      <td>
